...
class Shop():
	...
	def add_vote(self, vote:str, sign:str, public_key:str):
		"""
		...
		"""
		v = Vote(vote, sign, public_key)
		self.votes.append(v.dict())
		return v.dict()

	def verify_votes(self):
		"""
		Checks the validity of all votes in the votes list 
		"""
		for v in self.votes:
			validity = key_pair.valid_sign(v['vote'], v['sign'],v['public_key'])
			if(validity == False):
				print("the following vote is not legitimate:\n", v)
				return False
		return True
